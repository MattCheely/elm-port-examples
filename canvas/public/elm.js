...
var author$project$Model$init = {color: avh4$elm_color$Color$black, lastMouseDownPosition: elm$core$Maybe$Nothing};
...
var Elm_Canvas$raster_shapes$RasterShapes$LineStatics = F6(
...
var Elm_Canvas$raster_shapes$RasterShapes$initLine = F2(
	function (p, q) {
		var sy = (_Utils_cmp(p.y, q.y) > 0) ? 1 : (-1);
		var sx = (_Utils_cmp(p.x, q.x) > 0) ? 1 : (-1);
		var dy = A2(elm$core$Basics$composeL, elm$core$Basics$toFloat, elm$core$Basics$abs)(p.y - q.y);
		var dx = A2(elm$core$Basics$composeL, elm$core$Basics$toFloat, elm$core$Basics$abs)(p.x - q.x);
		var error = (_Utils_cmp(dx, dy) > 0) ? (dx / 2) : ((-dy) / 2);
		return _Utils_Tuple2(
			A6(Elm_Canvas$raster_shapes$RasterShapes$LineStatics, p.x, p.y, sx, sy, dx, dy),
			error);
	});
var Elm_Canvas$raster_shapes$RasterShapes$Position = F2(
	function (x, y) {
		return {x: x, y: y};
	});
var Elm_Canvas$raster_shapes$RasterShapes$calcError = F3(
	function (_n0, error, p) {
		var sx = _n0.sx;
		var sy = _n0.sy;
		var dx = _n0.dx;
		var dy = _n0.dy;
		var _n1 = (_Utils_cmp(error, dy) < 0) ? _Utils_Tuple2(dx, sy + p.y) : _Utils_Tuple2(0, p.y);
		var errY = _n1.a;
		var y = _n1.b;
		var _n2 = (_Utils_cmp(error, -dx) > 0) ? _Utils_Tuple2(-dy, sx + p.x) : _Utils_Tuple2(0, p.x);
		var errX = _n2.a;
		var x = _n2.b;
		return _Utils_Tuple2(
			(error + errX) + errY,
			A2(Elm_Canvas$raster_shapes$RasterShapes$Position, x, y));
	});
var Elm_Canvas$raster_shapes$RasterShapes$lineLoop = F4(
	function (statics, error, p, positions) {
		lineLoop:
		while (true) {
			var x = p.x;
			var y = p.y;
			if (_Utils_eq(x, statics.fx) && _Utils_eq(y, statics.fy)) {
				return A2(elm$core$List$cons, p, positions);
			} else {
				var _n0 = A3(Elm_Canvas$raster_shapes$RasterShapes$calcError, statics, error, p);
				var error_ = _n0.a;
				var q = _n0.b;
...
var Elm_Canvas$raster_shapes$RasterShapes$line = F2(
	function (p0, p1) {
		var _n0 = A2(Elm_Canvas$raster_shapes$RasterShapes$initLine, p1, p0);
		var statics = _n0.a;
		var error = _n0.b;
		return A4(Elm_Canvas$raster_shapes$RasterShapes$lineLoop, statics, error, p0, _List_Nil);
	});
var author$project$Model$clearMouseDownPosition = function (model) {
	return _Utils_update(
		model,
		{lastMouseDownPosition: elm$core$Maybe$Nothing});
};
var author$project$Model$setLastMouseDownPosition = F2(
	function (newLastMouseDownPosition, model) {
		return _Utils_update(
			model,
			{
				lastMouseDownPosition: elm$core$Maybe$Just(newLastMouseDownPosition)
			});
	});
var author$project$Ports$ColorPixels = function (a) {
	return {$: 'ColorPixels', a: a};
};
...
var author$project$Ports$encodeTo255 = A2(
	elm$core$Basics$composeL,
	A2(elm$core$Basics$composeL, elm$json$Json$Encode$int, elm$core$Basics$round),
...
var author$project$Ports$encodeColor = function (color) {
	...
	return elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'red',
				author$project$Ports$encodeTo255(red)),
				_Utils_Tuple2(
				'green',
				author$project$Ports$encodeTo255(green)),
				_Utils_Tuple2(
				'blue',
				author$project$Ports$encodeTo255(blue)),
				_Utils_Tuple2(
				'alpha',
				author$project$Ports$encodeTo255(alpha))
			]));
};
var author$project$Ports$encodePixel = function (_n0) {
	var color = _n0.a;
	var position = _n0.b;
	return elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'position',
				author$project$Data$Position$encode(position)),
				_Utils_Tuple2(
				'color',
				author$project$Ports$encodeColor(color))
			]));
};
...
var author$project$Ports$toCmd = F2(
...
var author$project$Ports$send = function (msg) {
	...
	return A2(
		...
		'colorPixels',
		A2(elm$json$Json$Encode$list, author$project$Ports$encodePixel, pixels));
};
...
var author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'MouseDownOnCanvas':
				var positionOnElement = msg.a;
				return _Utils_Tuple2(
					A2(author$project$Model$setLastMouseDownPosition, positionOnElement, model),
					author$project$Ports$send(
						author$project$Ports$ColorPixels(
							_List_fromArray(
								[
									_Utils_Tuple2(model.color, positionOnElement)
								]))));
			case 'MouseMoveOnCanvas':
				var positionOnElement = msg.a;
				var _n1 = model.lastMouseDownPosition;
				if (_n1.$ === 'Just') {
					var lastMouseDownPosition = _n1.a;
					return _Utils_Tuple2(
						A2(author$project$Model$setLastMouseDownPosition, positionOnElement, model),
						author$project$Ports$send(
							author$project$Ports$ColorPixels(
								A2(
									elm$core$List$map,
									elm$core$Tuple$pair(model.color),
									A2(Elm_Canvas$raster_shapes$RasterShapes$line, positionOnElement, lastMouseDownPosition)))));
				} else {
					return _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
				}
			default:
				return _Utils_Tuple2(
					author$project$Model$clearMouseDownPosition(model),
...
var author$project$Style$pBasic = rtfeldman$elm_css$Css$batch(
	_List_fromArray(
		[
...
var author$project$Style$globals = rtfeldman$elm_css$Css$Global$global(
	_List_fromArray(
		[
			...
			_List_fromArray(
				[author$project$Style$pBasic]))
		]));
var author$project$Data$Position$onTargetFromMouseEvent = function (_n0) {
	var offsetPos = _n0.offsetPos;
	var _n1 = offsetPos;
	var offsetX = _n1.a;
	var offsetY = _n1.b;
	return {
		x: elm$core$Basics$floor(offsetX),
		y: elm$core$Basics$floor(offsetY)
	};
};
var author$project$Msg$MouseDownOnCanvas = function (a) {
	return {$: 'MouseDownOnCanvas', a: a};
};
var author$project$Msg$MouseMoveOnCanvas = function (a) {
	return {$: 'MouseMoveOnCanvas', a: a};
};
var author$project$Msg$MouseUpOnCanvas = {$: 'MouseUpOnCanvas'};
...
var author$project$View$drawingCanvas = A2(
			...
			_List_fromArray(
				[
					...
					mpizenberg$elm_pointer_events$Html$Events$Extra$Mouse$onDown(
						A2(elm$core$Basics$composeL, author$project$Msg$MouseDownOnCanvas, author$project$Data$Position$onTargetFromMouseEvent))),
					rtfeldman$elm_css$Html$Styled$Attributes$fromUnstyled(
					mpizenberg$elm_pointer_events$Html$Events$Extra$Mouse$onMove(
						A2(elm$core$Basics$composeL, author$project$Msg$MouseMoveOnCanvas, author$project$Data$Position$onTargetFromMouseEvent))),
					rtfeldman$elm_css$Html$Styled$Attributes$fromUnstyled(
					mpizenberg$elm_pointer_events$Html$Events$Extra$Mouse$onUp(
						elm$core$Basics$always(author$project$Msg$MouseUpOnCanvas)))
				]),
			_List_Nil)
		]));
...
var author$project$View$centerTextStyle = rtfeldman$elm_css$Css$textAlign(rtfeldman$elm_css$Css$center);
...
var author$project$View$summary = A2(
	rtfeldman$elm_css$Html$Styled$p,
	_List_fromArray(
		[
			rtfeldman$elm_css$Html$Styled$Attributes$css(
			_List_fromArray(
				[author$project$View$centerTextStyle]))
		]),
	_List_fromArray(
		[
			rtfeldman$elm_css$Html$Styled$text('Click on the canvas, and drag, to draw.')
		]));
var author$project$View$title = A2(
	...
	_List_fromArray(
		[
			rtfeldman$elm_css$Html$Styled$text('Elm Canvas Ports Experiment')
		]));
...
var author$project$View$view = function (model) {
	return {
		body: A2(
			...
			_List_fromArray(
				[author$project$Style$globals, author$project$View$title, author$project$View$drawingCanvas, author$project$View$summary])),
